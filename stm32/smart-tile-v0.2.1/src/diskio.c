...
static void spi_set_speed(enum sd_speed speed) {
	...
	SPI_Init(SPI_SD, &SPI_InitStruct);
...
static const char *r1_strings[7] = { "in idle", "erase reset",
		"illegal command", "communication crc error", "erase sequence error",
...
static const char *r2_strings[15] = { "card is locked",
		"wp erase skip | lock/unlock cmd failed", "error", "CC error",
		"card ecc failed", "wp violation", "erase param",
		"out of range | csd overwrite", "in idle state", "erase reset",
		"illegal command", "com crc error", "erase sequence error",
		"address error", "parameter error", };
...
static int sd_read_csd(hwif *hw) {
	...
	if ((buf[0] >> 6) + 1 == 1) {
		/* CSD v1 */
		printf(
				"CSD: CSD v%i taac:%02x, nsac:%i, tran:%02x, classes:%02x%x, read_bl_len:%i, "
				...
				(buf[0] >> 6) + 1, buf[1], buf[2], buf[3], buf[4], buf[5] >> 4,
				1 << (buf[5] & 0xf), /* classes, read_bl_len */
				buf[6] >> 7, (buf[6] >> 6) & 1, (buf[6] >> 5) & 1,
				(buf[6] >> 4) & 1,
				(buf[6] & 0x3) << 10 | buf[7] << 2 | buf[8] >> 6, /* c_size */
				(buf[8] & 0x38) >> 3, buf[8] & 0x07, buf[9] >> 5,
				(buf[9] >> 2) & 0x7,
				1 << (2 + (((buf[9] & 3) << 1) | buf[10] >> 7)), /* c_size_mult */
				(buf[10] >> 6) & 1, ((buf[10] & 0x3f) << 1 | buf[11] >> 7) + 1, /* erase sector size */
				(buf[11] & 0x7f) + 1, /* write protect group size */
				buf[12] >> 7, 1 << ((buf[12] >> 2) & 7),
				1 << ((buf[12] & 3) << 2 | buf[13] >> 6), /* write_bl_len */
				(buf[13] >> 5) & 1, buf[14] >> 7, (buf[14] >> 6) & 1,
				(buf[14] >> 5) & 1, (buf[14] >> 4) & 1,
				(buf[14] >> 2) & 3 /* file format */);

		capacity = (((buf[6] & 0x3) << 10 | buf[7] << 2 | buf[8] >> 6) + 1)
				<< (2 + (((buf[9] & 3) << 1) | buf[10] >> 7))
				<< ((buf[5] & 0xf) - 9);
		...
		printf(
				...
				1 << (buf[5] & 0xf), /* classes, read_bl_len */
				buf[6] >> 7, (buf[6] >> 6) & 1, (buf[6] >> 5) & 1,
				(buf[6] >> 4) & 1, buf[7] << 16 | buf[8] << 8 | buf[9], /* c_size */
				(buf[10] >> 6) & 1, ((buf[10] & 0x3f) << 1 | buf[11] >> 7) + 1, /* erase sector size */
				(buf[11] & 0x7f) + 1, /* write protect group size */
				buf[12] >> 7, 1 << ((buf[12] >> 2) & 7),
				1 << ((buf[12] & 3) << 2 | buf[13] >> 6), /* write_bl_len */
...
static int sd_readsector(hwif *hw, u32 address, u8 *buf) {
	...
	return 0;
	fail: printf("failed to read sector %li, err:%i\n", address, r);
	return r;
}
...
static int sd_writesector(hwif *hw, u32 address, const u8 *buf) {
	...
	return 0;
	fail: printf("failed to write sector %li, err:%i\n", address, r);
	return r;
}
 ...
 */
typedef struct {
	DWORD sec :5;	// áèòû 0-4, 1 åä. = 2 ñåê
	DWORD min :6;	// áèòû 5-10
	DWORD hour :5;	// áèòû 11-15
	DWORD day :5;	// áèòû 16-20
	DWORD month :4;	// áèòû 21-24
	DWORD year :7;	// áèòû 25-31
} fat_time_t;
